...
# 주별 텍스트 요소 생성 함수
def create_text_element(key, province_contours):
...
def process_province(key, bgr_color, image, province_data):
    ...
    for province_name, info in province_data.items():
        if info["province"] == key:
            ...
            path_data = "M " + " ".join(f"{x-10},{y-10}" for x, y in contour[:, 0, :])
            province_path_elements.append(f'<path d="{path_data}" stroke="none" class="subdivision" id="subdivision{subdivision_id}" '
    ...
    # SVG 콘텐츠 생성
    svg_content = f"""
    <svg xmlns="http://www.w3.org/2000/svg" id="{key}">
        {''.join(province_path_elements)}
    </svg>
    """

    return svg_content, province_contours
...
def main():
    ...
    for key, bgr_color in province_colors.items():
        svg_content, province_contours = process_province(key, bgr_color, image, province_data)
        svg_elements.append(svg_content)
        text_elements.append(create_text_element(key, province_contours))
